...
export async function requestWikidata(language:string) {
  ...
  WHERE {
    ...
    OPTIONAL { 
      ?item p:P1343 ?documentationStatement. 
      ?documentationStatement pq:P2699 ?documentation.
      }
  ...
  GROUP BY ?item ?itemLabel ?description ?website ?viewing ?routing ?editor ?comparing ?hashtagTool ?monitoring ?changsetReview
  `;
  params["format"] = "json";
